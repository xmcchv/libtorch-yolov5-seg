...
//    const float pad_h = pad_info[1];
    ...
    // std::cout<<"detection========proto"<<std::endl;
    auto protos = output.toTuple()->elements()[1].toTensor();

    // result: n * 7
    // batch index(0), top-left x/y (1,2), bottom-right x/y (3,4), score(5), class id(6)
    auto result = PostProcessing(detections, protos, pad_info, scale, img.size(), conf_threshold, iou_threshold);
...
std::vector<float> Detector::LetterboxImage(const cv::Mat& src, cv::Mat& dst, const cv::Size& out_size) {
    ...
    std::vector<float> pad_info{static_cast<float>(left), static_cast<float>(top), scale,static_cast<float>(right),static_cast<float>(down)};
    return pad_info;
}


std::vector<std::vector<Detection>> Detector::PostProcessing(const torch::Tensor& detections, const torch::Tensor& protos,
                                                             const std::vector<float>& pad_info, float scale, const cv::Size& img_shape,
                                                             float conf_thres, float iou_thres) {
    constexpr int item_attr_size = 5;
    int batch_size = detections.size(0);
    // number of classes, e.g. 80 for coco dataset
    auto num_classes = detections.size(2) - item_attr_size - 32;

    float left = pad_info[0]; float top = pad_info[1];
    float right = pad_info[3]; float down = pad_info[4];

    auto conf_mask = detections.select(2, 4).ge(conf_thres).unsqueeze(2);

    auto seg_c = protos.size(1);
    auto mh = protos.size(2);
    auto mw = protos.size(3);
    ...
    for (int batch_i = 0; batch_i < batch_size; batch_i++) {
        ...
        auto det = torch::masked_select(detections[batch_i], conf_mask[batch_i]).view({-1, num_classes + item_attr_size + 32});
        ...
        det.slice(1, item_attr_size, item_attr_size + num_classes +32) *= det.select(1, 4).unsqueeze(1);
        ...
        auto mask = det.slice(1,item_attr_size + num_classes,item_attr_size + num_classes+32);

        // shape: n * 6, top-left x/y (0,1), bottom-right x/y (2,3), score(4), class index(5)
        det = torch::cat({box.slice(1, 0, 4), max_conf_score, max_conf_index, mask}, 1);
        ...
        // process mask    mask covariance matrix * proto
        torch::Tensor tindices = torch::tensor(nms_indices).to(torch::kCUDA);
        torch::Tensor t_masks = torch::index_select(mask, 0, tindices);  // 5 * 32
        auto tmp_protos = protos.to(torch::kFloat).view({seg_c, -1}); // 32 * 25600
        auto crop_masks = torch::matmul(t_masks,tmp_protos);
        crop_masks = torch::sigmoid(crop_masks);
        auto masks_bool = crop_masks.view({-1,mh,mw}).to(torch::kCPU);  // 5 160 160

        // crop mask
        auto mask_h = masks_bool.size(1);
        auto mask_w = masks_bool.size(2);

        torch::Tensor downsampled_bboxes = torch::index_select(det.index({"...",torch::indexing::Slice(0,4)}), 0, tindices).to(torch::kCPU);
        // Scale x-coordinates
        downsampled_bboxes.select(1, 0) *= ((float)mask_w / 640);
        downsampled_bboxes.select(1, 2) *= ((float)mask_w / 640);
        // Scale y-coordinates
        downsampled_bboxes.select(1, 3) *= ((float)mask_h / 640);
        downsampled_bboxes.select(1, 1) *= ((float)mask_h / 640);
        downsampled_bboxes = downsampled_bboxes.unsqueeze(2);  // [5 4 1]
        auto vbb = downsampled_bboxes.chunk(4, 1);
        auto x1 = vbb[0], y1=vbb[1], x2=vbb[2], y2=vbb[3];
        auto rbb = torch::arange(mask_w).to(torch::kFloat).unsqueeze(0).unsqueeze(1).to(torch::kCPU);
        auto cbb = torch::arange(mask_h).to(torch::kFloat).unsqueeze(0).unsqueeze(2).to(torch::kCPU);
        auto tmpcrop = ((rbb >= x1) * (rbb < x2) * (cbb >= y1) * (cbb < y2));
        masks_bool = masks_bool * tmpcrop;
        auto masks = masks_bool.unsqueeze(0);

        // upsample mask from 160*160  to  640*640
        masks = torch::nn::functional::interpolate(masks, torch::nn::functional::InterpolateFuncOptions().size(std::vector<int64_t>({640,640})).align_corners(false).mode(torch::kBilinear))[0];
        masks = masks.squeeze(0);
        masks = masks.gt(0.5);

        auto maskindex = masks.size(0);
        for(int i =0; i<maskindex; i++){
            auto a = masks.slice(0,i,i+1);
            cv::Mat protos(cv::Size(640,640), CV_8UC1, a.data_ptr());
            // remove padding
            cv::Rect roi(cv::Point(int(left),int(top)), cv::Point(int(640-right),int(640-down)));
            protos = protos(roi);
            //  640*640  to  img_shape
		    cv::resize(protos, protos, img_shape, cv::INTER_NEAREST);
            protos *= 255;
            det_vec[i].mask = protos.clone();
        }
        // resize bbox
        ScaleCoordinates(det_vec, left, top, scale, img_shape);
